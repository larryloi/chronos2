...
module Chronos2
  module Jobs
    class Traceable < Citrine::Runner::Job::CatchUp
      ...
      def traceable?; !options[:trace_attr].nil?; end
      ...
      def init_trace
        ...
        options[:trace_cycle] = 
          parse_time_interval(options[:trace_cycle]) if options[:trace_cycle]
      end
      ...
      def reset_trace
        trace[:started_at] = Time.parse(Time.now.strftime("%Y-%m-%d %H:%M:%S"))
      end
      ...
      def update_trace
        if success?
          traceable_dataset.each_value do |record|
            if record[options[:trace_attr]] > trace[:current_traced_at]
              ...
              trace[:traced_at] = Time.parse(trace[:current_traced_at].strftime("%Y-%m-%d %H:%M:%S"))
            end
          end
        end
        ...
        if caught_up?
          ...
          save_trace_log if stale_trace? and trace[:traced_at] != Time.at(0)
        end
      end
