...
module Chronos2
  module Jobs
    class Archive < Traceable
      ...
      def default_operation; :resolve_pending; end

      def archiving?; @operation == :archive_new; end
      ...
      def resolving_pending?; @operation == :resolve_pending; end
      ...
      def init_transform_processor
        unless options[:transform].nil?
          options[:transform] = eval("->(row, job) { #{options[:transform]}; row }")
        end
      end
      ...
      def update_summary_for_resolve_pending
        ...
        if pending_resolved?
          @summary += "No more pending records are left. Switched to archive new records."
        else
          @summary += "Continue to resolve pending records."
        end
      end
